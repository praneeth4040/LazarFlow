...
const DesignRenderer = ({ theme, data, lobby, width = SCREEN_WIDTH }) => {
    ...
    const getImageUrl = (url, theme) => {
        ...
        if (cleanUrl.includes('storage/v1/object/public/themes/')) {
            ...
            return `https://c977-49-204-99-215.ngrok-free.app/storage/themes/${cleanUrl.startsWith('/') ? cleanUrl.substring(1) : cleanUrl}`;
        }
        ...
        if (cleanPath.startsWith('optimized/')) {
            return `https://c977-49-204-99-215.ngrok-free.app/storage/themes/${cleanPath}`;
        }
        ...
        return `https://c977-49-204-99-215.ngrok-free.app/${cleanPath}`;
    };
