...
import { View, Text, StyleSheet, Image, TouchableOpacity, ScrollView, StatusBar, Platform, Dimensions } from 'react-native';
import { SafeAreaView } from 'react-native-safe-area-context';
import { ArrowLeft, Check, Share2, Heart, Palette } from 'lucide-react-native';
...
// Temporary bridge for legacy dataService reliance.
// Replace with appropriate repository call if dynamic images are loaded from backend.
const { getDesignImageSource } = require('../../lib/dataService');
...
const styles = StyleSheet.create({
    ...
    headerOverlay: { position: 'absolute', top: 0, left: 0, right: 0, flexDirection: 'row', justifyContent: 'space-between', paddingHorizontal: 20, paddingTop: Platform.OS === 'android' ? 40 : 10 },
    ...
    title: { fontSize: 28, fontFamily: Theme.fonts.outfit.bold, color: Theme.colors.textPrimary, flex: 1 },
    titleHeaderRow: { flexDirection: 'row', alignItems: 'flex-start', justifyContent: 'space-between', marginBottom: 12, gap: 12 },
    designIdBadge: { backgroundColor: 'rgba(26, 115, 232, 0.08)', paddingHorizontal: 8, paddingVertical: 4, borderRadius: 6, marginTop: 6 },
    designIdText: { fontSize: 10, color: Theme.colors.accent, fontFamily: 'monospace', letterSpacing: 0.5 },
    authorRow: { flexDirection: 'row', alignItems: 'center' },
    avatar: { width: 32, height: 32, borderRadius: 16, backgroundColor: Theme.colors.accent, alignItems: 'center', justifyContent: 'center', marginRight: 10 },
    avatarText: { color: '#fff', fontFamily: Theme.fonts.outfit.bold, fontSize: 14 },
    authorName: { fontSize: 16, color: Theme.colors.textSecondary, fontFamily: Theme.fonts.outfit.medium },
    descriptionSection: { marginBottom: 24 },
    sectionTitle: { fontSize: 18, fontFamily: Theme.fonts.outfit.bold, color: Theme.colors.textPrimary, marginBottom: 12 },
    ...
    useButton: { backgroundColor: Theme.colors.accent, height: 56, borderRadius: 28, flexDirection: 'row', alignItems: 'center', justifyContent: 'center', gap: 8, shadowColor: Theme.colors.accent, shadowOffset: { width: 0, height: 4 }, shadowOpacity: 0.3, shadowRadius: 8, elevation: 8 },
    useButtonText: { color: '#fff', fontSize: 18, fontFamily: Theme.fonts.outfit.bold }
});
